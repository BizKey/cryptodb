...
> **Статус:** скелет + Docker-окружение PostgreSQL с тюнингом под слабый хост.
> Стек приложения и инструмент миграций пока не определены; прикладного кода в репозитории нет.
...
└── db/
    ...
    ├── migrations/        # (будущие) эволюционные миграции схемы
    ├── postgresql.conf    # тюнинг PostgreSQL под 1 ядро / 1 ГБ RAM
    └── schema/            # (будущая) итоговая схема базы данных
```
...
Эволюционные изменения схемы после первого старта ведутся через миграции.
Инструмент (Flyway / Liquibase / goose / golang-migrate и т. п.) ещё **не выбран**;
файлы именуются `V<номер>__<описание>.sql` — см.
